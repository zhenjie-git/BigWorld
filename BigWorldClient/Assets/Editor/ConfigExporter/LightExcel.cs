...
{
    /// <summary>
    /// 极简 .xlsx 读取器：只支持导表工具所需的 OOXML 子集。
    /// 不依赖 NPOI/EPPlus 等第三方库，Unity Editor 自带的
    /// System.IO.Compression + System.Xml.Linq 即可解析。
    /// </summary>
    internal static class LightExcel
    {
        ...
        /// <summary>
        /// 读取指定 sheet 的所有非空单元格，key 为 "row:col"（都从 1 开始）。
        /// 空白单元格不会出现在结果里。
        /// </summary>
        public static Dictionary<string, string> ReadSheetCells(string xlsxPath, string sheetName)
        {
            ...
            using (var zip = ZipFile.OpenRead(xlsxPath))
            {
                ...
                // Target 常见写法：worksheets/sheet1.xml、/xl/worksheets/sheet1.xml、
                // xl/worksheets/sheet1.xml，统一成包内路径。
        ...
        static string CellToString(XElement cell, List<string> sharedStrings)
        {
            ...
            // 公式没有缓存值时读不到任何东西，按错误处理而不是静默当成空白。
